...
impl OrderBook {
    pub fn new(symbol: String, s: Side) -> Self {
        OrderBook {
            symbol,
    ...
    fn create_new_limit_order(&mut self, side: Side, price: u64, qty: u64) -> u64 {
    ...
    fn update_bbo(&mut self) {
        ...
        for (p, u) in self.ask_book.price_levels[*u].is_empty() {
    ...
    pub fn add_limit_order(&mut self, s: Side, price: u64, order_qty: u64) -> FillResult {
        ...
        match s {
            Side::Bid => {
                ...
                if let Some(mut x, _) = price_map_iter.next() {
                    while price >= *x {
                        ...
                        if matched_qty != 0 {
                            dbgp!("Match {} qty at level {}", matched_qty, x);
            ...
            Side::Ask => {
                ...
                let mut price_map_iter = price_levels.iter();
                ...
                if let Some(mut x, _) = price_map_iter.next_back() {
                    while price <= *x {
                        ...
                        if let Some(a) = price_map_iter.next_back() {
                            x = a;
                        } else {
                            break;
                        }
        ...
        fill_result.remaining_order_qty = remaining_order_qty;
        if remaining_order_qty != 0 {
            dbgp!("Remaining order qty {}", remaining_order_qty, price);
